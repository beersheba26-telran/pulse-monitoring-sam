...
"""Notifications jumps populator Lambda.

Receives SNS jump events and stores notification rows in PostgreSQL table
`notifications` using severity mapping by jump percentage:
- 40-50: MINOR
- 51-70: MAJOR
- >70: CRITICAL
"""
...
MAJOR_THRESHOLD_PERCENT = int(os.environ.get("JUMP_MAJOR_THRESHOLD_PERCENT", "50"))
CRITICAL_THRESHOLD_PERCENT = int(os.environ.get("JUMP_CRITICAL_THRESHOLD_PERCENT", "70"))


def _to_severity(previous_value: int, current_value: int) -> str:
	severity = "MINOR"
	jump_percent = abs(current_value - previous_value) / previous_value * 100
	if jump_percent > CRITICAL_THRESHOLD_PERCENT:
		severity = "CRITICAL"
	elif jump_percent > MAJOR_THRESHOLD_PERCENT:
		severity = "MAJOR"
...
def _build_notification(payload: dict) -> dict:
	...
	previous_value = payload["previous_pulse_value"]
	current_value = payload["current_pulse_value"]
	timestamp = payload["timestamp"]
	logger.debug(f"Building notification payload for device_id={device_id}, previous_value={previous_value}, current_value={current_value}, timestamp={timestamp}")

	severity = _to_severity(previous_value, current_value)
	created_at = datetime.fromtimestamp(timestamp, tz=timezone.utc)
	text = (
		f"Pulse jump detected for device {device_id}: "
